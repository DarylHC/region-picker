...
Component({
  ...
  methods: {
    ...
    _getProvinces : function(){
      return wx.getStorageSync('provinces').allProvinces;
    },

    _getCitiesByProvinceIndex:function(provinceIndex){
      let provinces = [];
      provinces = wx.getStorageSync('provinces').allProvinces;
    ...
    _getCitiesByProvinceID:function(provinceID){
      //从缓存中获取所有省份下的所有城市
      let allCities = wx.getStorageSync('cities').allCities;
    ...
    _getCountiesByCityID : function(cityID){
      let allCounties = wx.getStorageSync('counties').allCounties;
      let counties = [];
